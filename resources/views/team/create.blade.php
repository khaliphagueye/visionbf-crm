        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-slate-900 border border-amber-500/20 p-6 rounded-xl shadow-2xl">
                <form method="POST" action="{{ route('teams.store') }}">
                    ...
                    <div class="mb-6">
                        <label for="description" class="block text-sm font-medium text-slate-300">Description (Optionnel)</label>
                        <textarea id="description" name="description" rows="3"
                            class="mt-1 block w-full rounded-lg bg-slate-950 border-amber-500/30 text-slate-100 focus:border-amber-500 focus:ring-amber-500 text-sm">{{ old('description') }}</textarea>
