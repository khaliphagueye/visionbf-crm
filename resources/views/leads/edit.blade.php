<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            Modifier la Fiche : <span class="text-amber-600 font-bold">{{ $lead->raison_sociale }}</span>
        </h2>
    </x-slot>
                ...
                <form action="{{ route('leads.update', $lead) }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            <div>
                                ...
                                <input type="text" name="raison_sociale"
                                    value="{{ old('raison_sociale', $lead->raison_sociale) }}" required
                            ...
                            <div>
                                ...
                                <input type="text" name="telephone" value="{{ old('telephone', $lead->telephone) }}"
                                    required
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500">
                            ...
                            <div>
                                ...
                                <input type="text" name="siret" value="{{ old('siret', $lead->siret) }}"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500">
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-gray-700">Gérant / Contact</label>
                                <input type="text" name="gerant" value="{{ old('gerant', $lead->gerant) }}"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500">
                            </div>
                            ...
                            <div>
                                ...
                                <input type="text" name="adresse" value="{{ old('adresse', $lead->adresse) }}"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500">
                            </div>
                            ...
                            <div>
                                ...
                                <input type="text" name="code_postal"
                                    value="{{ old('code_postal', $lead->code_postal) }}"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500">
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-gray-700">Ville</label>
                                <input type="text" name="ville" value="{{ old('ville', $lead->ville) }}"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500">
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <select name="product_type"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500">
                                    <option value="Lanterneau" {{ old('product_type', $lead->product_type) == 'Lanterneau' ? 'selected' : '' }}>Lanterneau</option>
                                    <option value="Energie" {{ old('product_type', $lead->product_type) == 'Energie' ? 'selected' : '' }}>Énergie</option>
                                    <option value="Autre" {{ old('product_type', $lead->product_type) == 'Autre' ? 'selected' : '' }}>Autre</option>
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="number" step="0.01" name="superficie_lanterneau"
                                    value="{{ old('superficie_lanterneau', $lead->lanterneau->superficie_lanterneau ?? '') }}"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="pb-6 mb-6 border-b border-gray-200">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Date & Heure de Rappel /
                                    RDV</label>
                                <input type="datetime-local" name="rappel_at"
                                    value="{{ old('rappel_at', $lead->rappel_at ? \Carbon\Carbon::parse($lead->rappel_at)->format('Y-m-d\TH:i') : '') }}"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500">
                            </div>
                        </div>
                                ...
                                document.addEventListener('alpine:init', () => {
                                    Alpine.data('agentSelector', (config) => ({
                                        ...
                                        filterAgents() {
                                            ...
                                            // Réinitialise l'agent si celui sélectionné n'appartient pas à la nouvelle équipe choisie
                                            const agentExists = this.filteredAgents.some(a => a.id == this.selectedAgent);
                                            if (!agentExists && this.filteredAgents.length > 0) {
                                                this.selectedAgent = '';
                                            }
                                        }
                                    }));
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <textarea name="agent_comment" rows="3"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500">{{ old('agent_comment', $lead->agent_comment) }}</textarea>
                            </div>
                            ...
                            <div>
                                ...
                                <textarea name="confirmation_comment" rows="3"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500">{{ old('confirmation_comment', $lead->confirmation_comment) }}</textarea>
                            </div>
                        </div>
                    </div>
